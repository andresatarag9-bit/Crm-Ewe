...
<body>
    ...
    <main id="main-content">
        ...
        <section id="view-reports" class="hidden">
            ...
            <!-- Grid de Gráficas: Embudo + Llamadas/Reuniones -->
            <div class="charts-grid">
                <div class="chart-card">
                    <canvas id="funnelChart"></canvas>
                </div>
                <div class="chart-card">
                    <canvas id="callsMeetingsChart"></canvas>
                </div>
            </div>
    ...
    <script>
        ...
        let callsMeetingsInstance = null; // NUEVA VARIABLE PARA LA NUEVA GRÁFICA
        ...
        function renderReports() {
            ...
            // Render Charts (Embudo + Llamadas/Reuniones)
            // 1. Funnel Chart
            ...
            // 2. Calls vs Meetings Chart (NUEVO)
            const ctxCallsMeetings = document.getElementById('callsMeetingsChart').getContext('2d');
            
            // Calcular datos reales de interacciones
            let totalCalls = 0;
            let totalMeetings = 0;
            leads.forEach(l => {
                if(l.interactions) {
                    l.interactions.forEach(i => {
                        if(i.type === 'Llamada') totalCalls++;
                        if(i.type === 'Reunión') totalMeetings++;
                    });
                }
            });

            // Si no hay datos, generar datos de ejemplo para que se vea el gráfico la primera vez
            if(totalCalls === 0 && totalMeetings === 0) {
                totalCalls = 12;
                totalMeetings = 5;
            }

            if(callsMeetingsInstance) callsMeetingsInstance.destroy();
            callsMeetingsInstance = new Chart(ctxCallsMeetings, {
                type: 'bar',
                data: {
                    labels: ['Esta Semana'], // Se pueden agregar labels dinámicos si se desea
                    datasets: [
                        {
                            label: 'Llamadas',
                            data: [totalCalls],
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        title: { display: true, text: 'Llamadas vs Reuniones' }
                    }
                }
            });
            ...
            advisors.forEach(adv => {
                ...
                const calls = advLeads.reduce((acc, l) => acc + (l.interactions ? l.interactions.filter(i => i.type === 'Llamada').length : 0), 0);
                const meetings = interactions - calls;
        ...
        function deleteAdvisor(id) {
            if(confirm("¿Eliminar asesor? Los leads asignados quedarán sin asesor.")) {
                ...
                leads.forEach(l => {
                    // Limpiar asignación si el asesor eliminado coincide con el lead
